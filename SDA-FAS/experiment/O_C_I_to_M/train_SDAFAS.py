...
def get_args_parser():
    ...
    parser.add_argument('--source_model_transformer', default="/home/lyc/experiment/O_C_I_to_M/model_best_transformer.pth.tar",
                        help='path for the pretrained source transformer model')
    parser.add_argument('--source_model_embedder_classifier', default="/home/lyc/experiment/O_C_I_to_M/model_best.pth.tar",
                        help='path for the pretrained source embedder and classifier model')
    parser.add_argument('--dataset_path', default="/home/liuyuchen/MSU/MSUchoose_train_label_thirteen.json",
                        help='dataset path')
    parser.add_argument('--dataset_source_pseudo_path', default="/home/liuyuchen/MSU/MSU_train_pesudo.json",
                        help='path for the source pseudo label for dataset')
    parser.add_argument('--dataset_target_pseudo_path', default="/home/liuyuchen/MSU/MSU_train_pesudo1.json",
                        help='path for the source pseudo label for dataset')
    return parser
...
def test_and_save(args, train_dataloader, feature_transformer, model):
    ...
    final_train_json = []
    for i in range(len(prob_list_copy)):
        for haha in range(0,13):
            dict = {}
            dict['photo_path'] = original_all_label_json[(i)*13+haha]['photo_path']
            dict['GT_label'] = original_all_label_json[(i)*13+haha]['photo_label']
            if prob_list[i] > 0.5:
                dict['photo_label'] = 1
            else:
                dict['photo_label'] = 0
            dict['confidence'] = prob_list_copy[i]
            dict['photo_belong_to_video_ID'] = original_all_label_json[(i)*13+haha]['photo_belong_to_video_ID']
...
def test_and_save_target(args, train_dataloader, feature_transformer, model, model_target, log):
    ...
    for i in range(len(prob_list_copy)):
        for haha in range(0,13):
            dict = {}
            dict['photo_path'] = original_all_label_json[(i)*13+haha]['photo_path']
            dict['photo_label'] = source_pseudo_all_label_json[(i)*13+haha]['photo_label']
            dict['confidence'] = source_pseudo_all_label_json[(i)*13+haha]['confidence']
            if prob_list[i] > 0.5:
