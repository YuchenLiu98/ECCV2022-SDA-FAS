...
def get_args_parser():
    ...
    parser.add_argument('--source_model_transformer', default="/home/lyc/experiment/I_C_M_to_O/model_best_transformer.pth.tar",
                        help='path for the pretrained source transformer model')
    parser.add_argument('--source_model_embedder_classifier', default="/home/lyc/experiment/I_C_M_to_O/model_best.pth.tar",
                        help='path for the pretrained source embedder and classifier model')
    parser.add_argument('--dataset_path', default="/home/liuyuchen/OULU/OULUchoose_train_label_one.json",
                        help='dataset path')
    parser.add_argument('--dataset_source_pseudo_path', default="/home/liuyuchen/OULU/OULU_train_pesudo.json",
                        help='path for the source pseudo label for dataset')
    parser.add_argument('--dataset_target_pseudo_path', default="/home/liuyuchen/OULU/OULU_train_pesudo1.json",
                        help='path for the source pseudo label for dataset')
    return parser
...
def test_and_save(args, train_dataloader, feature_transformer, model):
    ...
    final_train_json = []
    for i in range(len(prob_list_copy)):
        for haha in range(0,1):
            dict = {}
            dict['photo_path'] = original_all_label_json[(i)*1+haha]['photo_path']
            dict['GT_label'] = original_all_label_json[(i)*1+haha]['photo_label']
            if prob_list[i] > 0.5:
                dict['photo_label'] = 1
            else:
                dict['photo_label'] = 0
            dict['confidence'] = prob_list_copy[i]
            dict['photo_belong_to_video_ID'] = original_all_label_json[(i)*1+haha]['photo_belong_to_video_ID']
...
def test_and_save_target(args, train_dataloader, feature_transformer, model, model_target, log):
    ...
    for i in range(len(prob_list_copy)):
        for haha in range(0,1):
            dict = {}
            dict['photo_path'] = original_all_label_json[(i)*1+haha]['photo_path']
            dict['photo_label'] = source_pseudo_all_label_json[(i)*1+haha]['photo_label']
            dict['confidence'] = source_pseudo_all_label_json[(i)*1+haha]['confidence']
            if prob_list[i] > 0.5:
...
def train(args):
    ...
    for iter_num in range(max_iter+1):
        ...
        alpha = float(2.0 / (1.0 + np.exp(-10 * iter_num / float(max_iter//2))) - 1.0)
        ######### cross-entropy loss #########
        cls_loss = 0
        count_number = 0
        total_TSE_loss = 0
        ...
        total_loss = cls_loss * (1-alpha) + alpha * cls_loss_target + 0.3*cda_loss + 0.1*total_TSE_loss
        total_loss.backward()
        ...
        loss_dino.update(0.1*total_TSE_loss.item())
