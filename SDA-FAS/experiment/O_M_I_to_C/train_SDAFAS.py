...
def get_args_parser():
    ...
    parser.add_argument('--dataset_path', default="/home/liuyuchen/CASIA-MFSD/CASIA_train_label_six.json",
                        help='dataset path')
    parser.add_argument('--dataset_source_pseudo_path', default="/home/liuyuchen/CASIA-MFSD/CASIA_train_pesudo.json",
                        help='path for the source pseudo label for dataset')
    parser.add_argument('--dataset_target_pseudo_path', default="/home/liuyuchen/CASIA-MFSD/CASIA_train_pesudo1.json",
                        help='path for the source pseudo label for dataset')
    return parser
...
def test_and_save(args, train_dataloader, feature_transformer, model):
    ...
    final_train_json = []
    for i in range(len(prob_list_copy)):
        for haha in range(0,6):
            dict = {}
            dict['photo_path'] = original_all_label_json[(i)*6+haha]['photo_path']
            dict['GT_label'] = original_all_label_json[(i)*6+haha]['photo_label']
            if prob_list[i] > 0.5:
                dict['photo_label'] = 1
            else:
                dict['photo_label'] = 0
            dict['confidence'] = prob_list_copy[i]
            dict['photo_belong_to_video_ID'] = original_all_label_json[(i)*6+haha]['photo_belong_to_video_ID']
...
def test_and_save_target(args, train_dataloader, feature_transformer, model, model_target, log):
    ...
    for i in range(len(prob_list_copy)):
        for haha in range(0,6):
            dict = {}
            dict['photo_path'] = original_all_label_json[(i)*6+haha]['photo_path']
            dict['photo_label'] = source_pseudo_all_label_json[(i)*6+haha]['photo_label']
            dict['confidence'] = source_pseudo_all_label_json[(i)*6+haha]['confidence']
            if prob_list[i] > 0.5:
