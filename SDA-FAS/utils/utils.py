...
def sample_frames(flag, num_frames, dataset_name):
    ...
    if dataset_name == 'CASIA_label':
        root_path = "/home/liuyuchen/CASIA-MFSD/" + dataset_name
    elif dataset_name == 'replay':
        root_path = "/home/liuyuchen/Replay/" + dataset_name
        if flag == 0 or flag == 1:
            num_frames = 1
        if flag == 3:
            num_frames = 4
    elif dataset_name == 'MSU':
        root_path = "/home/liuyuchen/MSU/" + dataset_name
        if flag == 3:
            num_frames = 13
    elif dataset_name == 'OULU':
        root_path = "/home/liuyuchen/OULU/" + dataset_name
        if flag == 3:
            num_frames = 1

    if(flag == 0): # select the fake images
        label_path = root_path + 'fake_label.json'
        save_label_path = root_path + 'choose_fake_label_L.json'
    elif(flag == 1): # select the real images
        label_path = root_path + 'real_label.json'
        save_label_path = root_path + 'choose_real_label_L.json'
    elif(flag == 2): # select all the real and fake images
        label_path = root_path + 'all_label.json'
        save_label_path = root_path + 'choose_all_label_L.json'
    elif(flag == 3):
        label_path = root_path + 'train_label.json'
        save_label_path = root_path + 'choose_train_label_L.json'
    else:
        label_path = root_path + 'test_label.json'
        save_label_path = root_path + 'choose_test_label_L.json'
        ...
        if (frame_prefix != saved_frame_prefix or i == length - 1):
            ...
            for j in range(num_frames):
                ...
                dict['photo_path'] = saved_frame_prefix + '/' + str(
                    single_video_frame_list[j * frame_interval]) + '.jpg'
...
def sample_frames_direct(save_label_path):
    '''
        from every video (frames) to sample num_frames to test
        return: the choosen frames' path and label
    '''
...
def save_checkpoint_transformer(save_list, is_best, model, gpus, checkpoint_path, best_model_path, filename='_checkpoint.pth.tar'):
    epoch = save_list[0]
    ...
    if is_best:
        shutil.copy(filepath, best_model_path + 'model_best_transformer_' + str(best_model_HTER) + '_' + str(epoch) + '.pth.tar')
